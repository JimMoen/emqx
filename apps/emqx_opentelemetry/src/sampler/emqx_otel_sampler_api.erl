...
paths() ->
    [
        "/opentelemetry/sample_white_lists/:type"
    ].

schema("/opentelemetry/sample_white_lists/:type") ->
    #{
        'operationId' => sample_whitelist,
        get => #{
            tags => ?API_TAGS,
...
sample_whitelist(get, #{bindings := #{type := Type}}) ->
    Res = [
        V
     || #?EMQX_OTEL_SAMPLER{type = {_, V}} <- emqx_otel_sampler:get_rules(Type)
    ],
    {200, Res};
sample_whitelist(post, #{bindings := #{type := Type}, body := Body}) when is_list(Body) ->
...
sample_whitelist(delete, #{bindings := #{type := Type}}) ->
